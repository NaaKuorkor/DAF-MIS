// resources/js/studentDashboard/announcementNotifications.js

export default function initStudentAnnouncementNotifications() {
    const bellIcon = document.getElementById('studentAnnouncementBellIcon');
    const dropdown = document.getElementById('studentAnnouncementDropdown');
    const unreadBadge = document.getElementById('studentAnnouncementUnreadBadge');
    const dropdownContent = document.getElementById('studentAnnouncementDropdownContent');
    const viewAllLink = document.getElementById('viewAllStudentAnnouncementsLink');

    if (!bellIcon || !dropdown) {
        console.warn('Student announcement notification elements not found');
        return () => {};
    }
     ...
     */
    window.viewStudentAnnouncementNotification = async function(announcementId) {
        try {
            // Mark as read
    ...
    function showAnnouncementModal(announcement) {
        ...
        const modalHTML = `
            <div id="studentAnnouncementNotificationModal" class="fixed inset-0 z-50 flex items-center justify-center" style="background: rgba(0, 0, 0, 0.5); backdrop-filter: blur(4px);">
                <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                    <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white z-10">
                        ...
                        <button onclick="document.getElementById('studentAnnouncementNotificationModal').remove()" class="text-gray-400 hover:text-gray-600 transition-colors">
                    ...
                    <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-end gap-3">
                        <button onclick="document.getElementById('studentAnnouncementNotificationModal').remove()" class="px-4 py-2 bg-purple-600 text-white text-sm font-medium rounded-lg hover:bg-purple-700 transition-all">
                            Close
                        </button>
                    </div>
                </div>
        ...
        const existingModal = document.getElementById('studentAnnouncementNotificationModal');
        ...
        if (viewAllLink) {
            viewAllLink.addEventListener('click', () => {
                ...
                if (window.displayContent) {
                    window.displayContent('/announcements');
                }
            });
        }
    ...
    return function cleanup() {
        ...
        closeDropdown();
        delete window.viewStudentAnnouncementNotification;
    };
}
